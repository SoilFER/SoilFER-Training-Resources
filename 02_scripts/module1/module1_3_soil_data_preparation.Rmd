...
raw_data <- read_excel("../../../01_data/module1/MIR_KANSAS_data_spectra_with_lat-long_depths_xlsx.xlsx", sheet = 1) 
...
summary(raw_data) # Summarize the data in each column
```

This dataset contains four types of information: site data (location and depth), laboratory data (measured soil properties), metadata (sample IDs), and spectral data (VIS-NIR reflectance values). In many cases, soil datasets lack spectral data. Understanding these components helps you organize the cleaning process systematically.
...
# Select only the columns needed for soil data preparation
site <- raw_data %>%
  select(
    rowID,
...
# Identify geographic coordinates outside valid ranges
out_of_bounds <- site %>%
  filter((lon < -180 | lon > 180) | (lat < -90 | lat > 90))

if (nrow(out_of_bounds) > 0) {
  # Common error: values stored in thousandths (e.g., 97500 instead of 97.5)
  # Attempt correction by dividing by 1000
  ...
  if (nrow(corrected_valid) > 0) {
    # Update site data with corrected values
    site <- site %>%
      left_join(
        out_of_bounds %>% select(rowID, lon_corrected, lat_corrected),
        by = "rowID"
      ) %>%
      mutate(
...
### Check 3: Remove exact duplicate site records
...
site <- site %>%
  group_by(ProfID) %>%
  filter(any(!is.na(top)) && abs(min(top, na.rm = TRUE)) < 0)  %>%
  ungroup()
...
### Extract and standardize laboratory columns

Extract laboratory variables from the raw dataset, standardize types to numeric, restrict records to those retained in the cleaned site dataset, and join lab data to the site dataset using `rowID` as the record key.
...
### Correction of out-of-bounds laboratory values
...
# # ---- SOC sign correction ----
extreme_idx <- site_lab$SOC < 0

if (any(extreme_idx, na.rm = TRUE)) {
  old_vals <- site_lab$SOC[extreme_idx]
  site_lab$SOC[extreme_idx] <- abs(site_lab$SOC[extreme_idx])
}

# ---- Mehlich-3 unit correction ----
extreme_idx <- !is.na(site_lab$Phosphorus_Mehlich3) &
               site_lab$Phosphorus_Mehlich3 > 2000

if (any(extreme_idx)) {
  old_vals <- site_lab$Phosphorus_Mehlich3[extreme_idx]
  site_lab$Phosphorus_Mehlich3[extreme_idx] <- site_lab$Phosphorus_Mehlich3[extreme_idx] / 1000
}

# Delete temporaty objects
rm(extreme_idx,old_vals)
```
...
for (property in names(out_of_bounds_issues)) {
  ...
  if (property %in% names(site_lab)) {
    old_vals <- site_lab[[property]][site_lab$rowID %in% rowIDs_with_issues]
    site_lab <- site_lab %>%
      mutate(
        "{property}" := ifelse(rowID %in% rowIDs_with_issues, NA, .data[[property]])
      )
    cat("Property", property, ":", length(rowIDs_with_issues), "values set to NA\n")
  }
}
```
...
## Detecting and resolving duplicate soil profiles {#duplicate-detection}
...
# Analyze depth consistency within each profile
...
# Select all horizons present in duplicated profiles
duplicates <- site_lab %>%
...
#### Action 1: Merge replicated horizons (same depth intervals)
...
## Action 2: Resolve multiple depth sequences
...
#### Check 1: Identify profiles with incomplete surface coverage

The objective is to subset profiles where the shallowest recorded depth is 0 cm.
...
properties_to_harmonize <- names(site_lab)[!names(site_lab) %in% c("rowID","ProfID","HorID","lon","lat","top","bottom","texture_sum","texture_valid")]
...
# Build the harmonization formula
fml <- as.formula(
  paste("ProfID ~", paste(properties_to_harmonize, collapse = " + "))
)

# Apply slab() for depth harmonization
...
The 5th to 95th percentile range provides a 90% confidence interval, quantifying uncertainty in the harmonized estimates.
...
### Processing standardized output
...
# Result: One row per profile-depth, with harmonized soil properties
head(KSSL_standardized)

```

The standardized dataset has now one row per profile per standard depth, with soil properties estimated at those depths using weighted averaging from the original horizon measurements. The final step is to save this dataset.
...
| KSSL_standardized              | Depth-harmonized dataset (0–30 cm; 30–60 cm) for Digital Soil Mapping       |
